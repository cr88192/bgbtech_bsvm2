...
int BS2PP_AddIncludeCache(char *str, byte *buf, int sz)
{
	...
	if(n>0)
	{
		bs2pp_cache_incname[hi]=bgbcc_strdup(str);
		bs2pp_cache_incbuf[hi]=buf;
		bs2pp_cache_incsz[hi]=sz;
		return(1);
	}
...
int BS2PP_Error(BS2CC_CompileContext *ctx, char *str, ...)
{
	...
	l=BGBCP_CalcLinenum(bs2pp_spos);
...
BS2CC_PpDef *BS2PP_AllocDefine(BS2CC_CompileContext *ctx)
{
	...
	tmp=bgbcc_malloc(sizeof(BS2CC_PpDef));
	memset(tmp, 0, sizeof(BS2CC_PpDef));
	return(tmp);
}
	...
	char *name, char *str)
{
	...
	cur=bgbcc_malloc2(sizeof(BS2CC_PpDef));
...
void BS2PP_Directive2(BS2CC_CompileContext *ctx, char *str)
{
	...
	if(!strcmp(b, "if"))
	{
		bs2pp_ifdname2[bs2pp_iflvl2]=bgbcc_strdup(str);
		bs2pp_iflvl2++;
		return;
	}
	...
	if(!strcmp(b, "ifdef"))
	{
		bs2pp_ifdname2[bs2pp_iflvl2]=bgbcc_strdup(b2);
		bs2pp_iflvl2++;
		return;
	}
...
void BS2PP_Directive(BS2CC_CompileContext *ctx, char *str)
{
	...
	if(!strcmp(b, "error"))
	{
		...
		printf("BS2PP_Directive: %s:%d: #error: %s\n",
			BGBCP_GetFilename(),
			BGBCP_CalcLinenum(bs2pp_spos), s);
		bs2pp_err++;
		...
		}else
		{
			if(def->value && !strncmp(def->value, "#ARCH:", 6))
			{
				...
				sprintf(bs2pp_obuf, "/*%d*/ __ifdef(%s) {\n",
					BGBCP_CalcLinenum(bs2pp_spos), def->value+6);
			}
			bs2pp_ifdname[bs2pp_iflvl]=bgbcc_strdup(b2);
			bs2pp_iflvl++;
		}
	...
	printf("BS2PP_Directive: %s:%d: Unsupported preproc directive %s\n",
		BGBCP_GetFilename(),
		BGBCP_CalcLinenum(bs2pp_spos), b);
	bs2pp_err++;
}
...
int BS2PP_BufferLine(BS2CC_CompileContext *ctx, char *b)
{
	...
	if(s1!=bs2pp_lfn)
	{
		sprintf(bs2pp_obuf, "/*\"%s\"%d*/ %s\n",
			s1, BGBCP_CalcLinenum(bs2pp_spos), b);
		bs2pp_lfn=s1;
	}else
	{
